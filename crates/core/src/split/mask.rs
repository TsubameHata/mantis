...
/// 
/// The value in its buffer should be valid `MaskValue`s by convention, which should be guaranteed by its callers, and is interpreted as its enum value, 
...
impl MaskLayer {
    ...
    /// 
    /// Panics if the value is incorrect.
    ...
    /// Maps the debug image generated by `MaskLayer::to_debug_image` back to `MaskLayer`.
    /// 
    /// Panics if the value is incorrect.
    /// 
    /// See the comments at `MaskLayer::to_debug_image` for details. 
    pub fn from_debug_image(img: GrayImage) -> Self {
...
impl FinalMask {
    ...
    pub fn x_range(&self) -> Option<(usize, usize)> {
        ...
        let mut right = 0;
        for (_y, row) in self.0.as_raw().chunks(width).enumerate() {
